...
int send_unit(trans_unit_t *tr_unit, trans_block_t *tr_block, alc_session_t *s,
  ...
  if(s->half_word) {
	...
	*(unsigned short*)def_lct_hdr = htons(*(unsigned short*)def_lct_hdr);
	...
	/* FEC Payload ID */
	//Malek El Khatib
	//printf("What is the next encoding symbol id: %u\n",tr_unit->esi); //Malek El Khatib 19.08.2014 Commented this
	//End
...
int alc_send(int s_id, int tx_mode, char *buf, int buf_len,
	...
	//Malek El Khatib 18.08.2014
	unsigned short max_nb_of_symbols =0;	/**<This is used to determine maximum nb of symbols to include in pakcet in case chunks of the object are sent at a time>*/
	//End
	...
	if(tx_mode != TX_THREAD) { /* We have only the base channel in this case */
		...
		if (toi !=FDT_TOI)
		{
			if (numEncSymbPerPacket > 0)
			{
				...
				{
					if (es_len*nb_of_symbols > MAX_SYMB_LENGTH_IPv4_FEC_ID_0_3_130)
			...
			else if (numEncSymbPerPacket == 0)
			{
				...
				max_nb_of_symbols = nb_of_symbols;
			}
		}
		//printf("What is the number of symbols and max number of symbols in a packet %u %u\n",nb_of_symbols, max_nb_of_symbols); //Malek El Khatib 19.08.2014 Commented this
		//END
	...
	//Malek El Khatib 14.08.2014
	//Determine size of segment metadate.
	//This value is used in null_fec.c to place the corresponding transport units at the end of list
	//This simulates the fact that the metadata is only generated after the whole media is generated
	segmentMetaData=0L;
	//Only if the TOI is in the input file do we introduce delay between the sending of consecutive chunks
	BOOL introduceDelay = FALSE;

	if (numEncSymbPerPacket == 0 && sendingTimesFile !="" && toi !=FDT_TOI)
	{
		if (getNextLine)
		{
			if( fgets (str, 1000,  FLUTEInputFile)!=NULL ) {
				...
				{
					tok = strtok(NULL, " ");
					segmentMetaData = strtoull (tok, NULL, 0);
					introduceDelay = TRUE;
				}
				else
					getNextLine = FALSE;
		...
		//fprintf(stderr,"******** introduceDelay %d, segmentMetaData %ull !\n",introduceDelay,segmentMetaData);
		//getchar();
	}
	//End
	...
	if(tx_mode != TX_THREAD) {
		...
		unsigned long long nb_of_es_in_chunk_to_send =0L;
		unsigned long long timeInUsec = 0L;

		if (toi !=FDT_TOI)
		{
			gettimeofday(&currentTime, NULL);
			timeInUsec = (unsigned long long)currentTime.tv_sec*1000000 + (unsigned long long)currentTime.tv_usec;
			fprintf(logFilePtr,"%llu ", timeInUsec);
		}
		//Increment i by nb_of_symbols
		//for(i = 0; i < tr_block->n; i++) {
		//printf("The total number of encoding symbols for this object is: %u\n",tr_block->n); //Malek El Khatib 19.08.2014 Commented this
		...
		for(i = 0; i < tr_block->n; i+=nb_of_symbols) {
			...
			if (introduceDelay)
			{
				//Number of units to send simultaneously
				tok = strtok(NULL, " \n");
				if (tok != NULL)
				{
					nb_of_es_in_chunk_to_send = strtoull (tok, NULL, 0);
				
					//When to send current chunk of units in absolute time
					tok = strtok(NULL, " \n");
					fileValue = strtoull (tok, NULL, 0);

					//Waiting to start sending
					gettimeofday(&currentTime, NULL);
					//printf("The times are: %llu %llu\n",fileValue,(unsigned long long)currentTime.tv_sec*1000000 + (unsigned long long)currentTime.tv_usec);
					if (fileValue > ((unsigned long long)currentTime.tv_sec*1000000 + (unsigned long long)currentTime.tv_usec))
					{
						...
						printf("Waiting for %llu microsenconds\n",elapsedTime); //Malek El Khatib 19.08.2014 Commented this
						usleep (elapsedTime);
					}
					introduceDelay = FALSE;		//Stop delaying until the whole chunk is sent
				}
				else	//We reached the end of line for this segment/object. Send remaining data (i.e. encoding symbols) with no delay
				{
					introduceDelay = FALSE;
					nb_of_es_in_chunk_to_send = 0L;
					//Send remaining data as fast as possible (i.e. Put as many es in packet as possible)
					nb_of_symbols = max_nb_of_symbols;
					getNextLine = TRUE;

				}
			}

			//printf("The nb_of_symbols to send in the next packet and the remaining es in chunk are: %u %u\n",nb_of_symbols,nb_of_es_in_chunk_to_send); //Malek El Khatib 19.08.2014 Commented this

			if (nb_of_es_in_chunk_to_send > 0L)
			{
				if (nb_of_es_in_chunk_to_send > max_nb_of_symbols)
				{
					nb_of_symbols = max_nb_of_symbols;
					nb_of_es_in_chunk_to_send -= nb_of_symbols;
				}
				else
				{
					nb_of_symbols = nb_of_es_in_chunk_to_send;
					nb_of_es_in_chunk_to_send -= nb_of_symbols;
					//It is time to read new chunk and send it at corresponding time from file
					introduceDelay = TRUE;
				}
			}

			
			//Note that metedata needs to be sent in a seperate packet since its encoding symbol id is not the continuation of the previous ones
			if ((symbolsSent < (tr_block->n - segmentMetaData)) && ((symbolsSent + nb_of_symbols) > (tr_block->n - segmentMetaData)))
				nb_of_symbols = tr_block->n - segmentMetaData - symbolsSent;

			else if ((symbolsSent >= (tr_block->n - segmentMetaData)) && ((symbolsSent + nb_of_symbols) > tr_block->n))
				nb_of_symbols = tr_block->n - symbolsSent;

			//printf("The nb_of_symbols to send in the next packet is: %u\n",nb_of_symbols); //Malek El Khatib 19.08.2014 Commented this

			//if ((symbolsSent + nb_of_symbols) > tr_block->n)
			//	nb_of_symbols = tr_block->n - symbolsSent;
			//if (i == (tr_block->n -1))
			//	nb_of_symbols = 1;
			...
			if(!randomloss(loss_prob)) {
				...
				//Malek El Khatib
				//Double check order of esi sent
				//printf("Sending Encoding Symbol ID: %llu\n", tr_unit->esi);
				...
				if(((toi == FDT_TOI && s->verbosity == 4) || (toi != FDT_TOI && s->verbosity > 1))) {
					...
					if(((tx_percent >= (get_object_last_print_tx_percent(s_id) + 30))
						|| (tx_percent == 100))) {
			...
			//Malek El Khatib 25.11.2014
			//Free data for all sent symbols in case multiple symbols are sent per packet
			int j =0;
			while (j<nb_of_symbols)
			{
				free(tr_unit->data);
			...
			//Commented on 25.11.2014
			//free(tr_unit->data);

			//Malek El Khatib 01.08.2014
			//Start
			//tr_unit++;
			//Commented below on 25.11.2014
		...
		//printf("The number of symbols and packets sent are: %d %d\n",symbolsSent,numOfPackets); //Malek El Khatib 19.08.2014 Commented this
                printf("The values of str and tok are: %s %s\n",&str, tok);
